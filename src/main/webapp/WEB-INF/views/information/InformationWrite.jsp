...
<body>
	<script>
		...
		var ckeditor_config = {
			...
			filebrowserUploadUrl : '/mtg/InformatioFile_upload.mtg', // 파일 업로드를 처리 할 경로 설정.
	...
	<div class="img">
		<span class="title">정보 포스트작성</span>
	</div>

	<form name="shit" action="/mtg/InformationWrite_ok.mtg" method="post">
	<input type="hidden" name="num" id="num" value="${object.num}"/>
	<input type="hidden" name="writer" id="writer" value="${object.writer}"/>
		<table align="center">
			<tr align="center" bgcolor="#ffffff">
				<td width="200" height="30">제목</td>
				<td width="600" height="30"><input type="text" value="${object.title}" id="title"
					name="title" theme="simple" maxlength="200" size="79px" /></td>
			</tr>
			<tr align="center" bgcolor="#ffffff">
				<td>대표이미지 등록하기</td>
				<td><textarea id="main_img" name="main_img" rows="10" cols="20"
						style="width: 766px; height: 412px;">${object.main_img}</textarea></td>
			</tr>
			<tr align="center" bgcolor="#ffffff">
				<td width="200" height="30">내용</td>
				<td width="600" height="30"><textarea id="cont" name="content"
						cols="95px" rows="20px">${object.content}</textarea></td>
			</tr>
			<tr>
				<td align="right" colspan="2"><input type="submit" id="save"
					value="등록"> <input type="button" value="목록"
					onclick="javascript:location.href='InformationList.mtg'"></td>
			</tr>
		</table>
	</form>
...
{
	 num ="${object.num}";
	  if (num > 0) document.shit.action = '/mtg/infoUpdate.mtg';
	  else document.shit.num.value = 0;
